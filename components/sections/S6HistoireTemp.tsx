...
/**
 * VERSION TEMPORAIRE de la section "Notre histoire" — en attendant que toutes
 * les photos des étapes soient fournies. Identique au carousel S6Histoire, mais :
 *   - Toutes les étapes SAUF la dernière (2026) : pas d'image, pas de trait
 *     vertical → le texte occupe toute la largeur de la carte.
 *   - L'étape 2026 reste intacte (texte | trait doré | image).
 *
 * Pour restaurer la mise en page d'origine : dans app/page.tsx remettre
 * <S6Histoire /> à la place de <S6HistoireTemp />, et ajouter imageSrc aux
 * chapitres 2013 + 2015 dans data/home.ts.
 */
export default function S6HistoireTemp() {
  ...
  return (
    <section className="gm-s6 gm-s6--temp" data-gm-section="s6-histoire" data-gm-tone="dark" id="histoire">
      ...
      >
        {HISTOIRE_CHAPTERS.map((ch, i) => {
          ...
          const isLast = i === total - 1; // étape 2026 : conserve image + trait

          return (
            <article
              key={ch.date}
              className={`gm-s6__chapter${isLast ? '' : ' gm-s6__chapter--no-media'}`}
      ...
      <div className="gm-s6__outro">
        <p>Maintenant, il ne vous reste plus qu&apos;à rejoindre notre histoire.</p>
      </div>
    </section>
  );
}
